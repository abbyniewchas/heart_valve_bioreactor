...
//  Dynamixel.wheel(SERVO_ID,LEFT,0x3FF);              // Comman for Wheel mode, Move left at max speed  
  Dynamixel.servo(SERVO_ID,1200,0x100);   // Comman for servo mode, Move servo to angle 1(0.088 degree) at speed 100
  delay(4000);
...
//  Dynamixel.wheel(SERVO_ID,RIGHT,0x3FF);          // Comman for Wheel mode, Move right at max speed 
   Dynamixel.servo(SERVO_ID,700,0x3FF);  // Comman for servo mode, Move servo to max angle at max speed (angle
  delay(4000);
